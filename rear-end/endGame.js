//                     https://af6g9a.fn.thelarkcloud.com/endGame
///////////////////////
// request{
//   roomId:1111,
//   winners:[1,2,3],
    ...
    if(room)   //房间存在
    {
      if(params.winners.length>0)   //传入winners积分
      {
        for(var i in params.winners) //对winner分数进行累加
        {
          ...
          room.playerList[i].score++;
          room.running = false;
        }
  
        for(var i=0;i<room.playerList.length;i++) //存入分数
        {
          var nameInScores = await scores.where({name:room.playerList[i].name}).findOne();
          if(nameInScores)
          {
            nameInScores.score+= room.playerList[i].score;
            await scores.save(nameInScores);
          }
          else
          {
            await scores.save({'name':room.playerList[i].name,'score':room.playerList[i].score});
          }
